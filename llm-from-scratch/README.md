...
3. **Generation is quadratic because nothing remembers.** Every step re-runs the
   ...
   [`../context-caching/`](../week-06/context-caching/) is about — the same model, from
   the serving side.
...
3. **A KV cache**, then measure generation cost against length before and after.
   Then read [`../context-caching/`](../week-06/context-caching/), which is that idea
...
- [`context-caching/`](../week-06/context-caching/) — the same architecture from the
  serving side: KV caching, prefix reuse, paged memory
- [`contextcite/`](../week-06/contextcite/) — attributing a generated answer back to its
  context
- [`cuda-from-scratch/`](../week-07/cuda-from-scratch/) — the matmuls, on hardware that
  can actually run them
- [`vllm-engine/`](../week-12/vllm-engine/) — serving one of these at scale
