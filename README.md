...
- **[compression-lower-bounds/](compression-lower-bounds/)** - Model compression and computational lower bounds, kept as two deliberately separate tracks. `compression/`: the rate-distortion objective, scalar Lloyd-Max against the Panter-Dite asymptote, E8 lattice quantization and its 0.654 dB space-filling gain, the random Hadamard transform (including the adversarial input where the *deterministic* transform makes incoherence maximally worse), GPTQ with an exact closed-form error-feedback update, and a trellis quantizer with Viterbi encoding. `lower-bounds/`: the comparison-sorting bound, Strassen derived from the rank-7 decomposition of the 2x2 matmul tensor, Baur-Strassen, and the red-blue pebble game - plus one page of prose on why none of the four binds a transformer. No solutions/ directory, on purpose (11 graded checks via `python3 check.py`)
