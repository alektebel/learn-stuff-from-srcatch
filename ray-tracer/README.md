...
- **Pure Python, single-threaded.** Roughly 24,000 rays/second, which is about
  ...
  [`cuda-from-scratch/`](../week-07/cuda-from-scratch/) is about.
...
|---|---|---|
| the parallelism | one thread, one ray at a time | [`cuda-from-scratch/`](../week-07/cuda-from-scratch/) — the same loop, 10⁵ at once |
| the execution model | branches are free | [`compiler-and-vgpu/`](../week-03/compiler-and-vgpu/) — one PC for eight lanes, so they are not |
| the sampling | Monte Carlo over light paths | [`diffusion-models/`](../week-16/diffusion-models/) — Monte Carlo over noise schedules |
