...
8. Compute concrete examples: ℚ(√2, √3), cyclotomic extensions
...
def Splits (E : Type) [Field E] [Algebra F E] (f : Polynomial F) : Prop :=
  ∃ (c : E) (roots : List E), 
    (∀ x, Polynomial.eval x (Polynomial.map (algebraMap F E) f) = 
          c * (roots.map (fun r => (x - r))).prod) ∧
    c ≠ 0
...
structure SplittingField (f : Polynomial F) where
  ...
  adjoin_roots : ∀ (K : Type) [Field K] [Algebra F K],
    Splits K f → 
    (∃ φ : carrier →ₐ[F] K, Function.Surjective φ) ∨
    (∀ r : carrier, (∃ g : Polynomial F, Polynomial.eval r (Polynomial.map (algebraMap F carrier) g) = 0 ∧ 
                                         g ∣ f) → 
     r ∈ Algebra.adjoin F {x : carrier | ∃ g, g ∣ f ∧ Polynomial.eval x (Polynomial.map (algebraMap F carrier) g) = 0})
...
-- If f splits in E and K ⊆ E contains all roots, then K = E
theorem splitting_field_minimal (f : Polynomial F) (E : SplittingField f)
    ...
    (h_splits : Splits K f)
    (h_roots : ∀ r : E.carrier, Polynomial.eval r (Polynomial.map (algebraMap F E.carrier) f) = 0 → 
               ∃ k : K, algebraMap K E.carrier k = r) :
    Function.Surjective (algebraMap K E.carrier) :=
  sorry
  -- Hint: E is generated by roots of f
  -- All roots are in K, so K maps onto E

-- TODO 1.4: Example - Splitting field of X² - 2 over ℚ is ℚ(√2)
example : ∃ E : Type, ∃ [Field E] [Algebra ℚ E],
    Splits E (Polynomial.X ^ 2 - Polynomial.C 2) ∧
    FiniteDimensional.finrank ℚ E = 2 :=
  sorry
  -- E = ℚ(√2) = {a + b√2 | a, b ∈ ℚ}
  -- X² - 2 = (X - √2)(X + √2) over E
  -- [E : ℚ] = 2
...
theorem linear_splits (a b : F) (ha : a ≠ 0) : 
...
theorem has_root_in_adjoin_root (f : Polynomial F) (hirr : Irreducible f) :
    ∃ r : adjoin_root f hirr, Polynomial.eval r (Polynomial.map (algebraMap F (adjoin_root f hirr)) f) = 0 :=
  sorry
...
theorem factor_by_root {E : Type} [Field E] [Algebra F E] (f : Polynomial F) (r : E)
    ...
    ∃ g : Polynomial E, Polynomial.map (algebraMap F E) f = (Polynomial.X - Polynomial.C r) * g ∧
                        g.degree = f.degree - 1 :=
  sorry
...
theorem splitting_field_exists (f : Polynomial F) (hf : f ≠ 0) :
    ∃ E : Type, ∃ [Field E] [Algebra F E], Splits E f :=
  sorry
  -- Proof by strong induction on degree:
  -- Base: deg ≤ 1, already splits over F
  -- Step: Take irreducible factor g of f
  --       Adjoin root in E₁ = F[X]/(g)
  --       f factors as (X - r) · f' over E₁
  --       By IH, ∃ E splitting field of f' over E₁
  --       Then E is splitting field of f over F

-- TODO 2.8: Prove splitting field is finite dimensional
theorem splitting_field_finite_dimensional (f : Polynomial F) (hf : f ≠ 0)
    (E : Type) [Field E] [Algebra F E] (h : Splits E f) :
    FiniteDimensional F E :=
  sorry
  -- Construct as tower: F = E₀ ⊆ E₁ ⊆ ... ⊆ Eₙ = E
  -- Each Eᵢ₊₁ = Eᵢ(rᵢ) where rᵢ is root of some factor
  -- [Eᵢ₊₁ : Eᵢ] ≤ deg(f)
  -- So [E : F] ≤ (deg f)! by tower law
...
-- Key lemma: If φ : F → F' and f splits over both E and E',
-- then φ extends to an isomorphism E ≅ E'
theorem extend_hom_to_splitting_field
    ...
    (h_min : ∀ r : E, Polynomial.eval r (Polynomial.map (algebraMap F E) f) = 0 → 
             r ∈ Algebra.adjoin F {x : E | Polynomial.eval x (Polynomial.map (algebraMap F E) f) = 0})
    (h_min' : ∀ r : E', Polynomial.eval r (Polynomial.map (algebraMap F E') f) = 0 → 
              r ∈ Algebra.adjoin F {x : E' | Polynomial.eval x (Polynomial.map (algebraMap F E') f) = 0}) :
    ∃ φ : E ≃ₐ[F] E', True :=
  sorry
  ...
  -- 4. φ determined by where it sends roots

-- TODO 3.2: Uniqueness up to isomorphism (main theorem)
theorem splitting_field_unique (f : Polynomial F) (hf : f ≠ 0)
    (E₁ E₂ : Type) [Field E₁] [Field E₂] [Algebra F E₁] [Algebra F E₂]
    (h₁ : Splits E₁ f) (h₂ : Splits E₂ f)
    (min₁ : ∀ r : E₁, Polynomial.eval r (Polynomial.map (algebraMap F E₁) f) = 0 → 
            r ∈ Algebra.adjoin F {x : E₁ | Polynomial.eval x (Polynomial.map (algebraMap F E₁) f) = 0})
    (min₂ : ∀ r : E₂, Polynomial.eval r (Polynomial.map (algebraMap F E₂) f) = 0 → 
            r ∈ Algebra.adjoin F {x : E₂ | Polynomial.eval x (Polynomial.map (algebraMap F E₂) f) = 0}) :
    Nonempty (E₁ ≃ₐ[F] E₂) :=
  ...
  -- Get F-algebra isomorphism E₁ ≃ₐ[F] E₂

-- TODO 3.3: Isomorphism preserves all field structure
-- This is automatic from the definition of ≃ₐ[F]
theorem splitting_field_iso_preserves_structure 
    (f : Polynomial F) (E₁ E₂ : Type) [Field E₁] [Field E₂] [Algebra F E₁] [Algebra F E₂]
    (φ : E₁ ≃ₐ[F] E₂) :
    (∀ a b, φ (a + b) = φ a + φ b) ∧
    (∀ a b, φ (a * b) = φ a * φ b) ∧
    (∀ a : F, φ (algebraMap F E₁ a) = algebraMap F E₂ a) :=
  sorry
  -- These follow from φ being an algebra isomorphism
...
def IsNormal (E : Type) [Field E] [Algebra F E] : Prop :=
  ∀ f : Polynomial F, 
    Irreducible f →
    (∃ r : E, Polynomial.eval r (Polynomial.map (algebraMap F E) f) = 0) →
    Splits E f
...
theorem normal_iff_splitting_field [FiniteDimensional F E] (E : Type) [Field E] [Algebra F E] :
    IsNormal E ↔ 
    ∃ f : Polynomial F, (∀ r : E, Polynomial.eval r (Polynomial.map (algebraMap F E) f) = 0 → 
                         r ∈ Algebra.adjoin F {x : E | Polynomial.eval x (Polynomial.map (algebraMap F E) f) = 0}) ∧
                        Splits E f :=
  sorry
...
theorem normal_tower (K E : Type) [Field K] [Field E] [Algebra F K] [Algebra F E] [Algebra K E]
    ...
    IsNormal E :=
  sorry
  -- If F ⊆ K ⊆ E with both extensions normal...
  -- Wait, this is not quite right!
  -- Correct statement: E/F normal iff E/K normal AND K/F normal? No!
  -- Actually: If E/F normal and K ⊆ E, then E/K NOT necessarily normal
  -- Example: ℚ ⊆ ℚ(∛2) ⊆ ℚ(∛2, ω∛2, ω²∛2)
  -- Last extension is normal over ℚ but ℚ(∛2) is not!
...
def IsSeparable (f : Polynomial F) : Prop :=
  ∀ E : Type, ∀ [Field E] [Algebra F E],
    Splits E f →
    (∀ r : E, (Polynomial.map (algebraMap F E) f).Roots.count r ≤ 1)

-- TODO 5.1: Separable iff coprime with derivative
theorem separable_iff_coprime_derivative (f : Polynomial F) (hf : f ≠ 0) :
    IsSeparable f ↔ Polynomial.gcd f (Polynomial.derivative f) = 1 :=
  sorry
  -- Key fact: r is repeated root iff r is root of both f and f'
  -- f has repeated roots iff gcd(f, f') has degree ≥ 1
...
theorem irreducible_separable_of_char_zero [CharZero F] (f : Polynomial F) 
    ...
    IsSeparable f :=
  sorry
  -- If f irreducible and char(F) = 0, then f' ≠ 0 (degree drops by 1)
  -- gcd(f, f') divides f
...
-- Over 𝔽ₚ(t), the polynomial Xᵖ - t is irreducible but not separable
example {p : ℕ} [Fact (Nat.Prime p)] :
    ∃ F : Type, ∃ [Field F], ∃ f : Polynomial F,
      Irreducible f ∧ ¬IsSeparable f :=
  sorry
  -- Let F = 𝔽ₚ(t), f = Xᵖ - t
  -- In splitting field, Xᵖ - t = (X - r)ᵖ where rᵖ = t
  -- This is because (X - r)ᵖ = Xᵖ - rᵖ in characteristic p
...
theorem separable_map (f : Polynomial F) (h : IsSeparable f)
    {E : Type} [Field E] (φ : F →+* E) :
    IsSeparable (f.map φ) :=
  sorry
  -- Roots of f.map φ correspond to roots of f
...
-- An element α is SEPARABLE over F if its minimal polynomial is separable
def IsSeparableElement (E : Type) [Field E] [Algebra F E] (α : E) : Prop :=
  ∃ f : Polynomial F, 
    (Polynomial.eval α (Polynomial.map (algebraMap F E) f) = 0) ∧
    (∀ g : Polynomial F, Polynomial.eval α (Polynomial.map (algebraMap F E) g) = 0 → f ∣ g) ∧
    IsSeparable f

-- An extension E/F is SEPARABLE if every element is separable
def IsSeparableExtension (E : Type) [Field E] [Algebra F E] : Prop :=
  ∀ α : E, IsSeparableElement E α
...
theorem separable_tower (K E : Type) [Field K] [Field E] [Algebra F K] [Algebra F E] [Algebra K E]
  ...
  -- Minimal poly of α ∈ E over F divides product of minimal polys over K and of K-coeffs over F
...
class IsPerfectField (F : Type) [Field F] : Prop where
  all_irreducible_separable : ∀ f : Polynomial F, Irreducible f → IsSeparable f
...
instance finite_field_perfect [Fintype F] : IsPerfectField F where
  all_irreducible_separable := sorry
  -- Key: Frobenius map x ↦ xᵖ is an automorphism of finite fields
  -- If f not separable, then f = g(Xᵖ) for some g
...
def IsGalois (E : Type) [Field E] [Algebra F E] : Prop :=
  IsNormal E ∧ IsSeparableExtension E ∧ Algebra.IsAlgebraic F E
...
theorem galois_iff_splitting_field (E : Type) [Field E] [Algebra F E] [FiniteDimensional F E] :
    IsGalois E ↔
    ∃ f : Polynomial F, IsSeparable f ∧ Splits E f ∧
    (∀ r : E, Polynomial.eval r (Polynomial.map (algebraMap F E) f) = 0 → 
     r ∈ Algebra.adjoin F {x : E | Polynomial.eval x (Polynomial.map (algebraMap F E) f) = 0}) :=
  sorry
...
-- TODO 7.4: Quadratic extensions are Galois over characteristic ≠ 2
theorem quadratic_galois [Invertible (2 : F)] (E : Type) [Field E] [Algebra F E]
    (h : FiniteDimensional.finrank F E = 2) :
    IsGalois E :=
  sorry
  -- E = F(√d) for some d ∈ F
  -- Minimal poly is X² - d, which is separable if char ≠ 2
  -- X² - d splits over E as (X - √d)(X + √d)
...
-- TODO 8.1: ℚ(√2) is Galois over ℚ
example : ∃ E : Type, ∃ [Field E] [Algebra ℚ E],
    IsGalois E ∧ FiniteDimensional.finrank ℚ E = 2 :=
  sorry
  -- E = ℚ(√2) = {a + b√2 | a, b ∈ ℚ}
  -- Splitting field of X² - 2
  -- X² - 2 is irreducible over ℚ (Eisenstein with p = 2)
  -- X² - 2 is separable (char 0)
  -- Splits as (X - √2)(X + √2) over E
  -- [E : ℚ] = 2

-- TODO 8.2: ℚ(√2, √3) is Galois over ℚ
example : ∃ E : Type, ∃ [Field E] [Algebra ℚ E],
    IsGalois E ∧ FiniteDimensional.finrank ℚ E = 4 :=
  sorry
  -- E = ℚ(√2, √3) = {a + b√2 + c√3 + d√6 | a,b,c,d ∈ ℚ}
  -- Splitting field of (X² - 2)(X² - 3)
  -- Tower: ℚ ⊆ ℚ(√2) ⊆ ℚ(√2, √3)
  -- [ℚ(√2) : ℚ] = 2, [ℚ(√2, √3) : ℚ(√2)] = 2
  -- So [E : ℚ] = 4
  -- Galois group isomorphic to ℤ/2ℤ × ℤ/2ℤ

-- TODO 8.3: ℚ(∛2) is NOT Galois over ℚ
example : ∃ E : Type, ∃ [Field E] [Algebra ℚ E],
    ¬IsGalois E ∧ FiniteDimensional.finrank ℚ E = 3 :=
  sorry
  -- E = ℚ(∛2) = {a + b∛2 + c∛4 | a,b,c ∈ ℚ}
  -- Minimal poly of ∛2 is X³ - 2, which is irreducible (Eisenstein)
  -- [E : ℚ] = 3
  -- But X³ - 2 does NOT split over E!
  -- Roots are ∛2, ω∛2, ω²∛2 where ω = e^(2πi/3)
  -- E only contains ∛2 (real root), not complex roots
...
example (n : ℕ) (hn : n > 0) : 
    ∃ E : Type, ∃ [Field E] [Algebra ℚ E],
      IsGalois E ∧ 
      FiniteDimensional.finrank ℚ E = (Nat.totient n) :=
  sorry
  -- E = ℚ(ζₙ) where ζₙ = e^(2πi/n)
  -- Splitting field of Xⁿ - 1
  -- Xⁿ - 1 splits completely in E as ∏(X - ζₙᵏ) for k = 0...n-1
  -- All n-th roots of unity lie in E
  -- Minimal polynomial is Φₙ(X) (cyclotomic polynomial)
  -- [E : ℚ] = φ(n) where φ is Euler's totient function
  -- Galois group Gal(ℚ(ζₙ)/ℚ) ≅ (ℤ/nℤ)ˣ (units mod n)

-- TODO 8.5: Splitting field of X⁴ - 2 over ℚ
example : ∃ E : Type, ∃ [Field E] [Algebra ℚ E],
    IsGalois E ∧ FiniteDimensional.finrank ℚ E = 8 :=
  sorry
  -- E = ℚ(∜2, i) where ∜2 = ⁴√2 (real fourth root)
  -- Roots of X⁴ - 2 are: ∜2, i∜2, -∜2, -i∜2
  -- Need to adjoin both ∜2 and i
  -- [ℚ(∜2) : ℚ] = 4, [ℚ(∜2, i) : ℚ(∜2)] = 2
  -- So [E : ℚ] = 8
  -- Galois group is dihedral group D₄ (order 8)
...
3. NORMAL ⇔ SPLITTING FIELD: For finite extensions
...
5. GALOIS: E/F Galois ⇔ E is splitting field of separable polynomial
...
CONCRETE EXAMPLES:
- ℚ(√2): Splitting field of X² - 2, [E:ℚ] = 2, Galois
- ℚ(√2, √3): Splitting field of (X²-2)(X²-3), [E:ℚ] = 4, Galois
- ℚ(∛2): Degree 3, but NOT Galois (not normal - missing complex roots)
- ℚ(ζₙ): Cyclotomic extensions, [E:ℚ] = φ(n), Galois
...
- Normal subgroups ↔ Normal extensions (Galois correspondence)
...
- Prove Fundamental Theorem (bijection: subgroups ↔ intermediate fields)
